...
public class Config {
    ...
    static {
        ...
        MESSAGES_CHANGE = builder.comment("Sent when /skin set is dispatched").define("messages_change", "Skin change queued");
        MESSAGES_FULFILLED = builder.comment("Sent when skin change completes successfully").define("messages_fulfilled", "Skin has been applied.");
        MESSAGES_TIMEOUT = builder.comment("Sent when Mojang/MineSkin fetch exceeds timeout").define("messages_timeout", "Skin fetch timed out.");
        MESSAGES_ERROR = builder.comment("Sent on provider failure").define("messages_error", "Skin fetch failed.");
        MESSAGES_RESTORED_FROM = builder.comment("Sent when /skin clear restores the player's Mojang skin; %s = source username").define("messages_restored_from", "Skin restored from %s");
        MESSAGES_CLEARED_NO_PROFILE = builder.comment("Sent when /skin clear finds no Mojang profile").define("messages_cleared_no_profile", "Skin cleared (no Mojang profile found)");
        MESSAGES_NO_SOURCE = builder.comment("Sent when /skin source finds no saved skin").define("messages_no_source", "No source available");
        MESSAGES_PLAYER_ONLY = builder.comment("Sent when a non-player tries /skin").define("messages_player_only", "Player only command");
        MESSAGES_PERMISSION_DENIED = builder.comment("Sent when permission check fails").define("messages_permission_denied", "Permission denied");
        MESSAGES_COOLDOWN = builder.comment("Sent when rate-limit cooldown is active; %d = seconds remaining").define("messages_cooldown", "Please wait %ds before using /skin again");
        MESSAGES_RATE_LIMITED = builder.comment("Sent when sliding-window rate limit triggers").define("messages_rate_limited", "Too many /skin commands. Try again later.");
        MESSAGES_NO_SKIN_FOUND = builder.comment("Sent when /skin set mojang <name> 404s; %s = requested name").define("messages_no_skin_found", "No skin found for \"%s\"");
        MESSAGES_NO_SKIN_FOUND_PLAIN = builder.comment("Sent when generic skin lookup fails").define("messages_no_skin_found_plain", "No skin found");
        MESSAGES_MINESKIN_REJECTED = builder.comment("Sent when MineSkin returns 4xx for /skin set web").define("messages_mineskin_rejected", "MineSkin rejected the URL");
        MESSAGES_NO_RANDOM_USERNAME = builder.comment("Sent when RandomMojangSkin scraper fails").define("messages_no_random_username", "No random username available");
        MESSAGES_PROVIDER_NO_RESULT = builder.comment("Sent when provider chain returns empty").define("messages_provider_no_result", "Provider returned no result");
        MESSAGES_METRICS_TOP_PLAYERS = builder.comment("Sent by /skin metrics players").define("messages_metrics_top_players", "Top players by refresh count:");
        MESSAGES_METRICS_REFRESHES = builder.comment("Per-player suffix in /skin metrics players; %s = player name + count").define("messages_metrics_refreshes", " refreshes");
        MESSAGES_METRICS_NO_REFRESHES = builder.comment("Sent when /skin metrics players has no data").define("messages_metrics_no_refreshes", "(no refreshes recorded)");
        MESSAGES_METRICS_CLEANUP = builder.comment("Sent by /skin metrics cleanup; %d = pruned count").define("messages_metrics_cleanup", "Metrics cleanup: pruned %d stale player entries");
        MESSAGES_METRICS_RESET = builder.comment("Sent by /skin metrics reset").define("messages_metrics_reset", "Metrics reset");
        MESSAGES_DISCORD_ANNOUNCE = builder.comment("DiscordSRV announcement format; %s1 = player name, %s2 = skin source").define("messages_discord_announce", "**%s** changed their skin to: `%s`");
        builder.pop();
