...
/**
 * 1.6.4 server config — static fields + {@link #load(File)}, ported from the
 * forge-1.7.10 reference (PR #477) off the mc1.12.2 template.
 *
 * <p>Key set mirrors the 1.7.10 port exactly: LANGUAGE (server/admin
 * language; per-player locale resolution is post-1.13), the URL allowlist
 * (consumed by the MineSkin generator on GameProfile lanes — 1.6.4 has no
 ...
 *
 * <p>1.6.4-era API: {@code Configuration} lives at
 * {@code net.minecraftforge.common.Configuration} (the
 * {@code net.minecraftforge.common.config} package is 1.7+ — verified
 * against the vendored decompiled 9.11.1.1345 tree) and exposes only the
 * raw {@code get(category, key, default, comment)} overloads returning
 * {@code Property} — the typed getString/getBoolean/getInt convenience
 * methods do not exist yet. Values are read via
 * {@code Property.getString()/getBoolean()/getInt()/getStringList()}.
 *
 * <p>Category quirk (same as 1.7.10): this era's {@code Configuration}
 * lowercases category names on lookup (caseSensitiveCustomCategories=false
 * default — visible in {@code Configuration.get(...)}) while the parser
 * keeps file-case names, so the .cfg categories MUST be lowercase
 * ("messages"/"security"/"mojangcache"/"everlastingskins") or they
 * silently miss.
 */
