...
/**
 * 1.6.4 I18nUtils key resolution over the classpath {@code .lang} files —
 * mirror of the forge-1.7.10 I18nUtilsTest (PR #477). The test classpath
 * carries src/main/resources (harness sourceSets), so the same 11 locale
 * files resolve here. {@link #setUp()} reloads all locales so tests are
 * independent of any prior load state.
 */
public class I18nUtilsTest {
    ...
    public void unknownKey_returnsRawKey() {
        // 1.6.4 StringTranslate.translateKey returns the key itself when
        // absent (verified in the vendored tree), same as 1.7+.
