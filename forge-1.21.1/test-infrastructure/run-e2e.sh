#!/usr/bin/env bash
# forge-1.21.1/test-infrastructure/run-e2e.sh — thin lane wrapper for the
...
#   server  — PRODUCTION Forge 1.21.1 server: the pinned forge installer
...
#   client  — the real 1.21.1 client launched the standard ForgeGradle dev
...
#             1.21.1 client). The 1.21.1 Main has NO --server/--port arg
...
# ---------------------------------------------------------------------------
cd "$LANE_DIR"
if ! JAVA_HOME="$JAVA21_HOME" "$REPO_ROOT/gradlew" :forge-1.21.1:build --no-daemon --console=plain; then
...
# ---------------------------------------------------------------------------
INSTALLER_NAME="forge-1.21.1-52.1.16-installer.jar"
INSTALLER_URL="https://maven.minecraftforge.net/net/minecraftforge/forge/1.21.1-52.1.16/forge-1.21.1-52.1.16-installer.jar"
INSTALLER_SHA1="076aad17209fe7f153887f7f3992506ab58472f5"
...
level-type=flat
motd=EverlastingSkins E2E 1.21.1
max-tick-time=-1
EOF
...
(
    ...
    exec setsid "$E2E_JAVA" @user_jvm_args.txt \
        @libraries/net/minecraftforge/forge/1.21.1-52.1.16/unix_args.txt nogui
) > "$SERVER_LOG" 2>&1 &
SERVER_PID=$!
...
(
    ...
    exec setsid xvfb-run -a env \
        ...
        ./gradlew :forge-1.21.1:runClient \
...
e2e_log "PASS: real-client E2E (1.21.1) all green"
exit 0
