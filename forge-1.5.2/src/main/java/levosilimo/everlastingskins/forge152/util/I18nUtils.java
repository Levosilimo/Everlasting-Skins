...
/**
 * i18n service for the 1.5.2 lane — ported from the forge-1.6.4 reference
 * (PR #478), itself the forge-1.7.10 port (PR #477) verbatim with only the
 * package and the {@link StatCollector} import changed (1.5.2 MCP domain is
 * {@code net.minecraft.src.*}).
 *
 ...
 * (1.5.2 {@code StringTranslate.translateKey} returns the raw key when
 * absent), then the raw key. The 1.5.2 server has no per-player locale
 * surface (that arrives with the 1.13+ client information packet), so
 * lookups use the server/admin language from {@link Config#LANGUAGE} only.
 */
