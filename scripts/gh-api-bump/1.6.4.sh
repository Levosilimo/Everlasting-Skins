#!/usr/bin/env bash
# gh-api-bump/1.6.4.sh
#
# Add "Build (forge-1.6.4)" to the required_status_checks contexts on main
# (integration/m2-monorepo was retired 2026-08-07 — PR #314 — so it is NOT
# targeted), bumping the contract from 19 to 20 contexts (AGENTS.md
# required-check section).
#
# Phase-5 merge order (19th vs 20th ordinal): forge-26.1 took the 17th
# ordinal ("Build (26.1)"), forge-1.18.2 the 18th ("Build (forge-1.18.2)"),
# and forge-1.10.2 the 19th ("Build (forge-1.10.2)" — applied first via
# PR #368; all verified live on main 2026-08-09). THIS script therefore
# bumps 19 -> 20. Merge order decides, so re-verify the live contract
# before --apply (the guard enforces the expected baseline; if the live
# set ever drifts from the 19-context EXPECTED, re-baseline it first).
#
...
# branch protection in-repo"). Run this AT PR-OPEN time for the forge-1.6.4
...
#                expected 19-entry baseline (and must NOT already contain
#                Build (forge-1.6.4), i.e. idempotent re-run is a no-op).
...
#                head and confirm a check named "Build (forge-1.6.4)" exists.
#
# Usage (run from the repo root; read-only unless --apply):
#   bash gh-api-bump/1.6.4.sh --help
#   bash gh-api-bump/1.6.4.sh --dry-run
#   bash gh-api-bump/1.6.4.sh --apply
#   bash gh-api-bump/1.6.4.sh --verify [ref]
#
...
BRANCHES=("main")
CONTEXT="Build (forge-1.6.4)"
# Lane branch head used by --verify (override with $2).
LANE_REF_DEFAULT="feature/lane-forge-1.6.4"

# The 19-context baseline BEFORE this lane lands (AGENTS.md; ci.yml job names
...
# guard <branch> — refuses unless the branch has exactly the expected 19
# contexts; treats an already-applied 20-context set as an idempotent no-op.
guard() {
  local branch="$1"
  require_auth
  local current
  ...
  if ! diff <(printf '%s\n' "${EXPECTED[@]}" | sort) <(printf '%s\n' "$current") >/dev/null; then
    echo ":: [$branch] REFUSING: contexts differ from the expected 19-entry baseline" >&2
    echo "::   expected:" >&2
    printf '::     %s\n' "${EXPECTED[@]}" >&2
    echo "::   actual:" >&2
  ...
  echo ":: [$branch] guard OK — exactly the 19 expected contexts"
}
...
apply_one() {
  ...
  echo ":: [$branch] PATCH required_status_checks (19 -> 20 contexts)"
...
apply_all() {
  ...
  echo ":: done — 20 contexts now required on main. Verify after the"
