#!/usr/bin/env bash
# gh-api-bump/1.18.2.sh
#
# Add "Build (forge-1.18.2)" to the required_status_checks contexts on main
# (integration/m2-monorepo was retired 2026-08-07 — PR #314 — so it is NOT
# targeted), bumping the contract from 17 to 18 contexts (AGENTS.md
# required-check section).
#
# Phase-5 merge order (17th vs 18th ordinal): forge-26.1 merged first and
# took the 17th ordinal ("Build (26.1)" — verified live on main 2026-08-08;
# AGENTS.md's "stays at 16 until then" note is stale). THIS script therefore
# bumps 17 -> 18. Had 26.1 not landed, the baseline would be 16 and this
# would be the 17th context — merge order decides, so re-verify the live
...
# branch protection in-repo"). Run this AT PR-OPEN time for the forge-1.18.2
...
#                expected 17-entry baseline (and must NOT already contain
#                Build (forge-1.18.2), i.e. idempotent re-run is a no-op).
...
#                head and confirm a check named "Build (forge-1.18.2)" exists.
#
# Usage (run from the repo root; read-only unless --apply):
#   bash gh-api-bump/1.18.2.sh --help
#   bash gh-api-bump/1.18.2.sh --dry-run
#   bash gh-api-bump/1.18.2.sh --apply
#   bash gh-api-bump/1.18.2.sh --verify [ref]
#
...
BRANCHES=("main")
CONTEXT="Build (forge-1.18.2)"
# Lane branch head used by --verify (override with $2).
LANE_REF_DEFAULT="feature/lane-forge-1.18.2"

# The 17-context baseline BEFORE this lane lands (AGENTS.md; ci.yml job names
...
# guard <branch> — refuses unless the branch has exactly the expected 17
# contexts; treats an already-applied 18-context set as an idempotent no-op.
guard() {
  local branch="$1"
  require_auth
  local current
  ...
  if ! diff <(printf '%s\n' "${EXPECTED[@]}" | sort) <(printf '%s\n' "$current") >/dev/null; then
    echo ":: [$branch] REFUSING: contexts differ from the expected 17-entry baseline" >&2
    echo "::   expected:" >&2
    printf '::     %s\n' "${EXPECTED[@]}" >&2
    echo "::   actual:" >&2
  ...
  echo ":: [$branch] guard OK — exactly the 17 expected contexts"
}
...
apply_one() {
  ...
  echo ":: [$branch] PATCH required_status_checks (17 -> 18 contexts)"
...
apply_all() {
  ...
  echo ":: done — 18 contexts now required on main. Verify after the"
