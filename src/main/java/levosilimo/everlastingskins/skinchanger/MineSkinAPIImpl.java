...
public class MineSkinAPIImpl {
    ...
    public MineSkinResponse genSkin(String imageUrl, @Nullable SkinVariant skinVariant) {
        ...
        if(!Objects.equals(imageUrl, SRHelpers.sanitizeSkinInput(imageUrl))) return new MineSkinResponse(SkinCommand.mojangAPI.getSkin(SRHelpers.sanitizeSkinInput(imageUrl)).skinProperty(), null, null, null);
        int retryAttempts = 0;
        do {
            lock.lock();
            try {
