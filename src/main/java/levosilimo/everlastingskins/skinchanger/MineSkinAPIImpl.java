...
import static levosilimo.everlastingskins.EverlastingSkins.logger;

@RequiredArgsConstructor(onConstructor_ = @Inject)
public class MineSkinAPIImpl implements MineSkinAPI {
    private static final int MAX_RETRIES = 5;
    public static Marker WARNING = MarkerManager.getMarker("WARNING");
    ...
    public MineSkinResponse genSkin(String imageUrl, @Nullable SkinVariant skinVariant) {
        ...
        do {
            ...
            try {
                ...
                logger.debug(WARNING, "[ERROR] MineSkin Failed! IOException (connection/disk): (" + imageUrl + ")", e);
                throw new Error(e);
            } finally {
                lock.unlock();
            }
        ...
        throw new Error("error ms attempt failed");
    }
    ...
    private HttpResponse queryURL(String url, @Nullable SkinVariant skinVariant) throws IOException {
        for (int i = 0; true; i++) { // try 3 times if server not responding
            try {
                ...
                return  WebUtils.POSTRequest().execute(
                        MINESKIN_ENDPOINT,
