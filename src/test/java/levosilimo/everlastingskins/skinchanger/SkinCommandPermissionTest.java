...
class SkinCommandPermissionTest {
    ...
    // backend is registered — mirror the production bootstrap (EverlastingSkins).
    @BeforeEach
    @AfterEach
    void resetManager() {
    ...
    void canTargetOthers_nonOp_returnsFalse() {
        ...
        assertFalse(PermissionServiceManager.hasPermission(ctx.uuid(), ctx.opLevel(), "everlastingskins.command.skin.other"));
    }
    ...
    void canTargetOthers_op_returnsTrue() {
        ...
        assertTrue(PermissionServiceManager.hasPermission(ctx.uuid(), ctx.opLevel(), "everlastingskins.command.skin.other"));
    }
}
