...
class SkinCommandPermissionTest {
    ...
    @BeforeEach
    @AfterEach
    void resetPermissionBackend() {
        // :common's manager is fail-closed without a registered backend; the
        // old uninitialized-vanilla fallback is gone. Grant by op level like
        // production's EverlastingSkins.init() would.
        if (PermissionServiceManager.getActiveBackendName().equals("Not initialized")) {
