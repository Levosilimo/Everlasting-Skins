...
`forge-1.10.2` / `forge-1.7.10` / `forge-1.6.4` are out-of-band per-lane
...
never configures them. `forge-1.6.4` is the oldest target in the repo —
Gradle 4.4.1 + Java 8 + vendored SpecialSource 1.7.4 deobf harness (GTNH FG
1.2.11 cannot build 1.6.4; no MCP 1.6.4 channel exists). `forge-1.7.10` —
...
- **CI:** `.github/workflows/ci.yml` is a per-module matrix (PR #260,
  ...
  forge-1.6.4 (own wrappers), and the
  ...
  (`mc1.10.2-v*`), and the forge-1.6.4 lane adds `Build (forge-1.6.4)` +
  `publish-mc1_6_4` (`mc1.6.4-v*`).
