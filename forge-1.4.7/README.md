# EverlastingSkins — forge-1.4.7 lane

Out-of-band legacy lane: Minecraft 1.4.7 / FML 4.7 (`cpw.mods.fml`, `@Mod`/`@Mod.Init`) / pre-UUID (no GameProfile). Builds standalone with its own Gradle 4.4.1 wrapper on Java 8 via the vendored SpecialSource remap harness (no ForgeGradle); consumes `:common` by source-dir share. See `build.gradle` for the full toolchain rationale.

Build: `cd forge-1.4.7 && JAVA_HOME=<jdk8> ./gradlew build`
