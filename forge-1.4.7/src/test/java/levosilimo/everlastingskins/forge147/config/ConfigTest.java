...
/**
 * 1.4.7 Config load/parse round-trip + defaults, mirroring the forge-1.5.2
 * ConfigTest (PR #479) off the forge-1.6.4 / forge-1.7.10 / mc1.12.2
 * template. The .cfg fixture uses the era's lowercase categories and the
 * B:/S:/I: property-prefix format (Property.Type.getID() = first letter,
 * unchanged since 1.4 — verified in the vendored decompile).
 */
public class ConfigTest {

    /**
     * 1.4.7 quirks, all verified against the vendored decompiled tree
     * (identical to the 1.5.2/1.6.4 siblings):
     *
     ...
     * so load() is exercisable. 1.4.7's FMLInjectionData lives in
     * {@code cpw.mods.fml.relauncher}. No-op in production (FML sets it at
     * launch).
     *
     * <p>2. Configuration has a STATIC field sized off
     * {@code Item.itemsList.length} (decompiled line 30: {@code configMarkers}),
     * so merely loading the class initializes the whole MC item/block
     * registry. Item's static init is cyclic with Block's, so the JVM's
     * class-init order decides the outcome: triggering Item first NPEs,
     * triggering Block first (the real server-boot order) lets Item run to
     * completion inside the chain. Forcing {@code Block} init below
     * reproduces the boot order and the cycle resolves re-entrantly.
     */
    @BeforeClass
    ...
    public void load_missingFileWritesDefaults() throws Exception {
        ...
        // 1.4.7 Configuration has no hasChanged() — Config saves unconditionally,
        // so defaults are always persisted to disk.
