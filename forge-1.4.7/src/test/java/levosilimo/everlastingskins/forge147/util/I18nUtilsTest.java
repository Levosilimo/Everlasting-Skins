...
/**
 * 1.4.7 I18nUtils key resolution over the classpath {@code .lang} files —
 * mirror of the forge-1.5.2 / forge-1.6.4 / forge-1.7.10 I18nUtilsTest.
 * The test classpath carries src/main/resources (harness sourceSets), so
 * the same 11 locale files resolve here. {@link #setUp()} reloads all
 * locales so tests are independent of any prior load state.
 */
public class I18nUtilsTest {
    ...
    public void unknownKey_returnsRawKey() {
        // 1.4.7 StringTranslate.translateKey falls back to the key itself
        // (translateTable.getProperty(key, key)), verified in the vendored tree.
