...
/**
 * 1.4.7 permission adapter.
 *
 * <p>1.4.7 has no Forge PermissionAPI and no per-player permission-node
 ...
 * method 1.6.4/1.7.10 use; MCP 7.26a func_70003_b). The shared
 * {@code hasPermission(UUID, int, String)} contract is mapped by node suffix:
 * the node's required op level is checked against the player's actual op
 * level via {@code canCommandSenderUseCommand}.
 *
 * <p>Skin storage is keyed by UUID only (memory #1123), and 1.4.7 has no
 ...
 * derived UUID; 1.4.7 has no {@code getPlayerByUUID} (that is 1.7.10+).
 *
