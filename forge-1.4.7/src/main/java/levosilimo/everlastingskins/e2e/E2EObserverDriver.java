 ...
 * 7.26a) with zero reflection, mirroring {@link E2EDriver}. Like the actor,
 ...
 * MULTIMAPS; the @NetworkMod client spec registers first, so the real
 * handler runs before this driver's listener on the same packet — and the
 * injection is polled, not assumed, so listener order never matters).
 *
...
public final class E2EObserverDriver implements IScheduledTickHandler, IPacketHandler {
    ...
    static void install() {
        ...
        // Coexists with the real ClientSkinHandler: FML 4.7 keeps
