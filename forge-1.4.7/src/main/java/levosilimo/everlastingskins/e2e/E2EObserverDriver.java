 ...
 * gap (d), ported to the 1.4.7 MERGE model): an actor client runs
 ...
 * compiles DIRECTLY against the client classes (MCP 7.26a) with zero
 ...
 * Download-thread race: the 1.4.7 TDI keeps NO reference to its download
 ...
 * registration coexists with the real handler (FML 4.7 per-channel handler
 * multimaps). Auto-connect: like {@link E2EDriver#install()}, this driver
 * calls {@code Minecraft.setServer} itself (1.4.7's {@code Minecraft.main}
...
public final class E2EObserverDriver implements IScheduledTickHandler, IPacketHandler {
    ...
    static void install() {
        ...
        // Coexists with the real ClientSkinHandler: FML 4.7 keeps per-channel
    ...
    // ------------------------------------------------------------------
    // Join + target lookup (direct client references — MCP 7.26a surface).
    // ------------------------------------------------------------------
