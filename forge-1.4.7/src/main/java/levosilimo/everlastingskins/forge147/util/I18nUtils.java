...
package levosilimo.everlastingskins.forge147.util;
...
/**
 * i18n service for the 1.4.7 lane — ported from the forge-1.5.2 reference
 * (PR #479), itself the forge-1.6.4 (PR #478) / forge-1.7.10 (PR #477)
 * ports verbatim with only the package changed.
 *
 ...
 * (1.4.7 has StatCollector, unlike the 1.5.2/1.6.4-only assumption —
 * verified in the vendored tree; {@code StringTranslate.translateKey}
 * returns the raw key when absent), then the raw key. The 1.4.7 server has
 * no per-player locale surface (that arrives with the 1.13+ client
 * information packet), so lookups use the server/admin language from
 * {@link Config#LANGUAGE} only.
 */
