...
public class PropertyUtils {
    ...
    public static DecodedTextureProperty getSkinProfileData(CustomSkinProperty property) {
        String decodedString = new String(Base64.getDecoder().decode(propertyValue(property.getOriginalProperty())), StandardCharsets.UTF_8);

        return GSON.fromJson(decodedString, DecodedTextureProperty.class);
    }

    /**
     * Cross-version authlib {@link Property} value accessor. authlib exposes
     * {@code getValue()} up to the 1.12.2 era and {@code value()} on 1.21+;
     * the shared module must compile and run against both, so the modern
     * accessor is tried reflectively first and the legacy getter second.
     */
    public static String propertyValue(Property property) {
        return invokeAccessor(property, "value", "getValue");
    }

    /**
     * Cross-version authlib {@link Property} signature accessor; see
     * {@link #propertyValue(Property)}.
     */
    public static String propertySignature(Property property) {
        return invokeAccessor(property, "signature", "getSignature");
    }

    private static String invokeAccessor(Property property, String modernName, String legacyName) {
        try {
            Method modern = property.getClass().getMethod(modernName);
            return (String) modern.invoke(property);
        } catch (ReflectiveOperationException e) {
            try {
                Method legacy = property.getClass().getMethod(legacyName);
                return (String) legacy.invoke(property);
            } catch (ReflectiveOperationException e2) {
                throw new IllegalArgumentException(
                    "authlib Property exposes neither " + modernName + "() nor " + legacyName + "()", e2);
            }
        }
    }
