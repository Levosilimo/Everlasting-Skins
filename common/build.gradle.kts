plugins {
    `java-library`
    // no-mixin convention plugin: registers verifyNoMixin and wires it into
    // `build` so any Mixin usage (annotations, mixingradle, mixin {} blocks,
    // mixins.json bundling) fails the build. See build-logic/ for the plugin.
    id("no-mixin")
}
