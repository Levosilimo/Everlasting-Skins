...
### Build gates (no source changes)

- `no-mixin` convention plugin (`build-logic/`, precompiled script plugin,
  id `no-mixin`) — registers `verifyNoMixin` and wires it into `build`;
  fails on any Mixin usage (annotations, mixingradle classpath, mixin
  plugin id, `mixin {}` blocks, `everlastingskins.mixins.json` bundling).
  Consumers copy it into their own `buildSrc/` at M2 step 2.
- `scripts/assert-no-mixin.sh` — pre-flight gate for CI (mirrored in the
  `lint-no-mixin` workflow); requires zero Mixin usage.
- `build.gradle.kts` — added `-Xlint:deprecation`; with the existing
  `-Werror`, deprecated-API usage now fails the build explicitly.
