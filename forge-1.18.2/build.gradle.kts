// forge-1.18.2 — standalone Gradle build for the Minecraft 1.18.2 lane.
//
...
// 6.0.54 applied via the buildscript classpath (the 1.18.2 MDK pattern).
//
...
val minecraftVersion: String = requireNotNull(project.findProperty("minecraft_version")?.toString()) {
    "forge-1.18.2: 'minecraft_version' must be set in gradle.properties"
}
val forgeVersion: String = requireNotNull(project.findProperty("forge_version")?.toString()) {
    "forge-1.18.2: 'forge_version' must be set in gradle.properties"
}
...
java {
    // Forge 40 (1.18.2) ships Java 17 to end users (per the official MDK).
    toolchain {
...
dependencies {
    // The artifact is the 1.18.2-40.3.0 Forge userdev build; FG 6.0 wires
