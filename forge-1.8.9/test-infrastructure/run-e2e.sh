#!/usr/bin/env bash
# forge-1.8.9/test-infrastructure/run-e2e.sh — thin lane wrapper for the
...
# scenario SENDs "/skin set mojang Notch" through the console, and the
# server-side ES_E2E_SKIN sentinel is the primary assertion.
#
...
if [ -n "${JAVA_HOME:-}" ] && [ -x "$JAVA_HOME/bin/java" ]; then
    ...
    echo "[e2e:1.8.9][fail] Java 8 not found (set JAVA_HOME to a JDK 8)" >&2
    exit 3
fi
echo "[e2e:1.8.9] Java 8: $E2E_JAVA8"

# Build the lane
echo "[e2e:1.8.9] building lane..."
cd "$LANE_DIR"
if ! JAVA_HOME="$(dirname "$(dirname "$E2E_JAVA8")")" ./gradlew build --no-daemon; then
    echo "[e2e:1.8.9][fail] lane build failed (exit 3)" >&2
    exit 3
fi

MOD_JAR="$(ls build/libs/everlastingskins-*.jar 2>/dev/null | grep -v -- '-sources.jar' | head -1 || true)"
if [ -z "$MOD_JAR" ]; then
    echo "[e2e:1.8.9][fail] no mod jar in build/libs/" >&2
    exit 3
fi

# Invoke the shared orchestrator (headlessmc era)
export E2E_LANE="1.8.9"
