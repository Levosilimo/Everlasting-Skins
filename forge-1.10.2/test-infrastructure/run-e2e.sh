#!/usr/bin/env bash
# forge-1.10.2/test-infrastructure/run-e2e.sh — thin lane wrapper for the
...
#
# In-jar driver lane: 1.10.2 has no hmc-specifics module, so the lane's
# shipped E2EDriver (gated by -Deverlastingskins.e2e=true) sends /skin
# through the real chat surface and writes e2e-result.json into the
# gameDir; the scenario waits for the driver markers.
#
...
# Java 8 (hard requirement: launchwrapper + Gradle 4.10.3 die on 9+)
if [ -n "${JAVA_HOME:-}" ] && [ -x "$JAVA_HOME/bin/java" ]; then
    ...
    echo "[e2e:1.10.2][fail] Java 8 not found (set JAVA_HOME to a JDK 8)" >&2
    exit 3
fi
echo "[e2e:1.10.2] Java 8: $E2E_JAVA8"

# Build the lane (full build: the e2e jar must be the REOBF'd artifact —
# a bare `jar` produces the dev jar whose deobf names crash the obfuscated
# server at runtime, verified in the slice-2 trials)
echo "[e2e:1.10.2] building lane..."
cd "$LANE_DIR"
if ! JAVA_HOME="$(dirname "$(dirname "$E2E_JAVA8")")" ./gradlew build --no-daemon; then
    echo "[e2e:1.10.2][fail] lane build failed (exit 3)" >&2
    exit 3
fi

MOD_JAR="$(ls build/libs/everlastingskins-*.jar 2>/dev/null | grep -v -- '-sources.jar' | head -1 || true)"
if [ -z "$MOD_JAR" ]; then
    echo "[e2e:1.10.2][fail] no mod jar in build/libs/" >&2
    exit 3
fi

# Invoke the shared orchestrator (headlessmc era)
export E2E_LANE="1.10.2"
