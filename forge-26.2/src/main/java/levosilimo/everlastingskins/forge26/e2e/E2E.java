 ...
 * modern-injar pattern — 26.x port of the forge-1.21 reference
 * implementation), invoked from {@code EverlastingSkins()} (one line:
 * {@code E2E.install();}).
 *
 ...
 * </ul>
 *
 * <p>26.x deltas vs the 1.21 reference: the dist gate is unchanged
 * ({@code FMLEnvironment.dist} still exists on the unobfuscated line), but
 * the listener registration moved to EventBus 7 typed buses
 * ({@code TickEvent.ClientTickEvent.Post.BUS} / {@code ServerStartedEvent.BUS})
 * — there is no {@code MinecraftForge.EVENT_BUS} addListener path (that
 * field is an {@code EventBusMigrationHelper} now).
 *
 ...
 * .properties().get("textures")}) — authlib 7/9 GameProfile is a record, so
 * the accessor is {@code properties()} (not {@code getProperties()}). The
 * tab-list entry IS the received packet data
 * (ClientboundPlayerInfoUpdatePacket ADD_PLAYER), NOT a pixel injection.
 */
public final class E2E {
     ...
     * across runs and greppable in both logs.
     */
     ...
     * {@code --uuid} argument, so the seed, the login session and the
     * client-side assert all key on one UUID.
     */
