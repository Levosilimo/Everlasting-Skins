 ...
 * {@code modern-injar} pattern — 26.x port of the forge-1.21 reference
 * implementation; same official-mapped names on the unobfuscated line).
 *
 * <p>Shipped in the mod jar, gated at runtime by
 * {@code -Deverlastingskins.e2e=true} + {@code Dist.CLIENT} (see
 * {@link E2E#install()}). The lane wrapper boots the real 26.x client the
 * standard ForgeGradle way ({@code runClient} under xvfb + Mesa llvmpipe)
 * and passes the offline session via the launcher args; the 26.x client
 * Main auto-joins via the quick-play arg ({@code --quickPlayMultiplayer
 * 127.0.0.1:25565} — GameConfig.QuickPlayMultiplayerData → QuickPlay.connect
 * → ConnectScreen.startConnecting, bytecode-verified on the unobfuscated
 * 26.x client jar); this driver runs inside that client on the client tick
 * event.
 *
 ...
 * .properties().get("textures")}. The driver asserts that property is
 ...
 *
 * <p>26.x deltas vs the 1.21 reference:
 * <ul>
 *   <li>EventBus 7 typed-bus registration: {@code TickEvent.ClientTickEvent
 *       .Post.BUS.addListener(...)} — the Post record IS the event (no
 *       phase check; there is no {@code MinecraftForge.EVENT_BUS}
 *       addListener path).</li>
 *   <li>{@code Minecraft.gui.screen()} — 26.2 moved the active screen off
 *       the {@code Minecraft} field onto {@code Gui} (26.1 kept
 *       {@code mc.screen}; this port targets 26.2).</li>
 *   <li>authlib record accessors: {@code getProfile().properties()} (not
 *       {@code getProperties()}); {@code PropertyMap} is a Guava
 *       {@code ForwardingMultimap}, so {@code get("textures")} still returns
 *       a {@code Collection<Property>}.</li>
 * </ul>
 *
...
public final class E2EDriver {
    ...
    /** Test server the wrapper's quick-play arg dials (matches the wrapper). */
    ...
    public static void install() {
        ...
        // EventBus 7 typed-bus registration (26.x): the Post record carries
        // its own EventBus<Post>; no static-subscriber delta.
        TickEvent.ClientTickEvent.Post.BUS.addListener(E2EDriver::onClientTick);
        ...
        switch (nextPhase(phase, isJoined(mc), texturesPropertyContainsMarker(mc))) {
            case WAIT_JOIN -> {
                if (mc != null) {
                    ...
                    // the title screen).
                    String screenClass = mc.gui.screen() == null ? "<null>" : mc.gui.screen().getClass().getName();
                    if (!screenClass.equals(lastScreenClass)) {
                        lastScreenClass = screenClass;
                        EverlastingSkins.logger.info("ES_E2E_SCREEN={}", screenClass);
                        if (mc.gui.screen() instanceof DisconnectedScreen ds) {
                            logDisconnectReason(ds);
                        }
                    }
                    // Fallback only: the wrapper's quick-play arg joins before
                    // the title screen appears, so this normally never fires.
                    if (!connectStarted && mc.gui.screen() instanceof TitleScreen) {
                        autoConnect(mc);
                    }
                }
    ...
    private static void logDisconnectReason(DisconnectedScreen screen) {
        ...
        // trial log (unobfuscated runtime keeps the official field names).
        try {
    ...
    /**
     * Dial-the-server fallback (quick-play is the primary join on 26.x, so
     * this is the 1.21-parity safety net for a title-screen boot without a
     * connection). Gated on {@code TitleScreen}: the early loading screen is
     * also a non-null screen, and connecting while the datapack load is
     * still running gets clobbered by the startup flow (observed live on
     * 1.21: ES_E2E_CONNECT fired mid-load, the title screen then replaced
     * the connect screen, and no TCP dial ever reached the server).
     */
     ...
     * first (observed live on 1.21: the sentinel was present in the tab list
     * while the raw contains() stayed false).
     */
    ...
    private static String texturesProperty(Minecraft mc) {
        ...
        // authlib 7/9: GameProfile is a record (properties() accessor);
        // PropertyMap extends Guava ForwardingMultimap, so get(name)
        // returns a Collection (the textures property is single-valued in
        // practice).
        Collection<Property> textures = info.getProfile().properties().get("textures");
