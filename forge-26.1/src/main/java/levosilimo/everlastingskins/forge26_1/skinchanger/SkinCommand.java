...
import levosilimo.everlastingskins.forge26_1.util.JavaHttpClient;
...
public class SkinCommand {
    ...
    private static int sourceAction(CommandContext<CommandSourceStack> context, ServerPlayer target) {
        ...
        if (SkinRestorer.getSkinStorage().hasDefaultSkin(uuid)) {
            context.getSource().sendSuccess(() -> Component.literal(FEEDBACK_PREFIX + " " + target.getGameProfile().name()), false);
            return 1;
        }
